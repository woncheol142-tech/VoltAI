...
import type { DrawingPageMapDocument } from "./types.js";
...
function ensureSecurePageMapDirectory(projectRoot: string): string {
  let currentPath = projectRoot;
  for (const part of [".volt-ai", "page-maps"]) {
    currentPath = join(currentPath, part);
    try {
      const stats = lstatSync(currentPath);
      if (stats.isSymbolicLink()) {
        throw new Error("Drawing page map output path cannot include symbolic links");
      }
      if (!stats.isDirectory()) {
        throw new Error("Drawing page map output path must contain directories only");
      }
    } catch (error) {
...
function createTarget(
  projectRoot: string,
  document: DrawingPageMapDocument,
  outputName: string,
): { absolutePath: string; relativePath: string } {
  const safeName = sanitizeOutputName(outputName);
  const directory = ensureSecurePageMapDirectory(projectRoot);
  const sourcePathHash = createHash("sha256")
    .update(document.source.normalize("NFKC").replaceAll("\\", "/"), "utf8")
    .digest("hex")
    .slice(0, 12);
  const range = `p${String(document.startPage).padStart(3, "0")}-p${String(document.endPage).padStart(3, "0")}`;
  const fileName = `${safeName}-${sourcePathHash}-${range}.json`;
  return {
    absolutePath: join(directory, fileName),
    relativePath: posix.join(".volt-ai", "page-maps", fileName),
  };
}
...
function writeAtomically(absolutePath: string, content: string): void {
  ...
  try {
    ...
    try {
      ...
      if (!existing.isFile()) {
        throw new Error("Drawing page map output path must reference a regular file");
      }
    } catch (error) {
      if (!isMissingPathError(error)) throw error;
    }
...
export function writeDrawingPageMap(
  projectRoot: string,
  document: DrawingPageMapDocument,
  outputName: string,
): string {
  const target = createTarget(realpathSync(projectRoot), document, outputName);
  const stored: DrawingPageMapDocument = {
    ...
    indexedDrawingCount: document.indexedDrawingCount,
    mappingCount: document.mappingCount,
    unmatchedCount: document.unmatchedCount,
    coverageRatio: document.coverageRatio,
    mappings: document.mappings,
    unmatchedDrawingNumbers: document.unmatchedDrawingNumbers,
    duplicatePageMatches: document.duplicatePageMatches,
