...
const INPUT_FIELDS = new Set(["indexPath", "query", "limit", "filters"]);
...
export async function searchDrawings(
  ...
  const { indexPath, query, limit, filters } = validateInput(input);
  const document = loadDrawingIndex(projectRoot, indexPath);
  return searchDrawingIndex(document, { query, limit, filters });
}
