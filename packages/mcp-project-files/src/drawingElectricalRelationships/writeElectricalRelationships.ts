...
import { serializeElectricalRelationshipDocument } from "./serializeElectricalRelationships.js";
...
function ensureSecureDirectory(projectRoot: string): string {
  ...
  for (const part of [".volt-ai", "electrical-relationships"]) {
    currentPath = join(currentPath, part);
    try {
      const stats = lstatSync(currentPath);
      if (stats.isSymbolicLink()) {
        throw new Error("Electrical relationship output path cannot include symbolic links");
      }
      if (!stats.isDirectory()) {
        throw new Error(
          "Electrical relationship output path must contain directories only",
        );
      }
    } catch (error) {
...
function assertSafeTarget(absolutePath: string): void {
  try {
    ...
    if (!existing.isFile()) {
      throw new Error(
        "Electrical relationship output path must reference a regular file",
      );
    }
  } catch (error) {
    if (!isMissingPathError(error)) throw error;
...
export function writeElectricalRelationshipDocument(
  projectRoot: string,
  document: unknown,
  outputName: string,
): string {
